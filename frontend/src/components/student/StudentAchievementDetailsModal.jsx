...
export default function StudentAchievementDetailsModal({ isOpen, onClose, achievement }) {
...
const InfoCard = ({ icon, label, value, color }) => (
  <div className="space-y-1.5 p-4 rounded-2xl bg-slate-50/50 border border-slate-100/50 hover:bg-white hover:shadow-sm hover:border-slate-200 transition-all">
    <div className="flex items-center gap-2">
      <div className={`w-6 h-6 flex items-center justify-center rounded-lg ${color || 'text-emerald-500 bg-emerald-50'}`}>{icon}</div>
      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{label}</span>
    </div>
    <p className="text-sm font-bold text-slate-700 truncate px-1" title={value}>{value}</p>
  </div>
);
