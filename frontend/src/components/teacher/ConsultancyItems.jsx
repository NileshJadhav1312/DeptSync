...
export function ConsultancyFormModal({ isOpen, onClose, onSubmit, initialData }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto"><div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm" onClick={onClose} /><div className="relative w-full max-w-4xl bg-white rounded-[2rem] shadow-2xl max-h-[90vh] flex flex-col overflow-hidden"><h2 className="text-2xl font-bold mb-6">Consultancy Details</h2><form onSubmit={hSubmit} className="space-y-4"><input required name="title" value={formData.title} onChange={handleChange} className="input" placeholder="Consultancy Title" /><input required name="organization.name" value={formData.organization.name} onChange={handleChange} className="input" placeholder="Client / Organization" /><div className="grid grid-cols-2 gap-4"><input type="number" required name="revenueGenerated" value={formData.revenueGenerated} onChange={handleChange} className="input" placeholder="Revenue" /><select name="paymentStatus" value={formData.paymentStatus} onChange={handleChange} className="input"><option value="Pending">Pending</option><option value="Paid">Paid</option></select></div><div className="flex justify-end gap-2 pt-4"><button type="button" onClick={onClose} className="btn-secondary">Cancel</button><button type="submit" className="btn-primary">Save Consultancy</button></div></form></div></div>
  );
}
