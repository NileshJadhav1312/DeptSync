...
export default function ResearchPaperDetailsModal({ isOpen, onClose, paper }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-2xl bg-white rounded-2xl shadow-xl max-h-[90vh] flex flex-col">
        <div className="p-6 border-b border-slate-100 flex items-center justify-between">
          <h2 className="text-xl font-bold text-slate-900">Research Paper Details</h2>
        ...
        <div className="p-6 overflow-y-auto space-y-4">
          <div className="grid grid-cols-2 gap-y-4 gap-x-6 text-sm">
            <div className="col-span-2">
              ...
              <span className="font-bold text-slate-900 text-base">{paper.title}</span>
            </div>
            <div>
            ...
            <div>
              ...
              <span className="font-semibold text-slate-900">{paper.publicationType}</span>
            </div>
            <div>
              <span className="block font-medium text-slate-500 uppercase tracking-wide text-xs">Publication Level</span>
              <span className="font-semibold text-slate-900">{paper.publicationLevel}</span>
            </div>
            <div>
              <span className="block font-medium text-slate-500 uppercase tracking-wide text-xs">Publisher</span>
              <span className="font-semibold text-slate-900">{paper.publisherName || "N/A"}</span>
            </div>
            <div>
            ...
            <div>
              ...
              <span className="font-semibold text-slate-900 break-all">{paper.doi || "N/A"}</span>
            </div>
          </div>

          <div className="pt-4 border-t border-slate-100">
             <span className="block font-medium text-slate-500 uppercase tracking-wide text-xs mb-1">Authors</span>
             <span className="font-semibold text-slate-900">{paper.authors?.join(", ") || "None"}</span>
          </div>
        </div>
